...
- [ ] `r/fred.R` (CPI + PMMS series) — no raw files needed, start here
- [ ] `r/acs_costs.R` (rents/values/burden tables incl. trends) — no raw files needed
- [ ] `r/fmr.R` (FMR/SAFMR tidy)
- [ ] `r/nhpd.R`: Fauquier properties/units by program + expirations
- [ ] `r/costar.R`: adapt faar pattern incl. CPI adjustment
- [ ] `r/mls.R`: monthly + annual output frames; splice VAR/GP/MLS; listings snapshot; rentals
- [ ] Validation: 2025 median price ~$645,250; ~218 active listings (Jul 2026 snapshot); SF rent ~$2,450; ~750 assisted units (13 NHPD properties)
...
| 5 | Market data | prereqs complete | 2026-07-09 | Sonnet 4.6 |

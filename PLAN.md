...
6. **[Amendment, Session 11b — Jonathan-approved]** Household-formation **range** (`fig-need-range`): hold WC 2050 pop fixed, vary only avg household size (2.78 / 2.70 / 2.60) → ~270–360 units/yr band; 269 headline as the conservative floor, GP's 307 inside; high end reframed as convergence toward state/national declining size (Fauquier's own size is flat, verified 2.76→2.76→2.78).
7. **[Amendment, Session 11b — Jonathan-approved]** Components-of-need **scope ladder** (`fig-scope-ladder`): cumulative building bar — new households → + vacancy → + existing backlog (data-grounded sub-80% rental deficit 1,059 + assumption-flagged 1/3-of-ownership-shortfall 2,539) → + replace aging stock (assumption, 0.25%/yr). Presents need as scope, not a single number.
...
| 11 | Ch: projections + conclusions + data notes | in progress (S11a done) | 2026-07-14 | Opus 4.8 |
...
- **2026-07-14** — Session 11 **part a** (`projections.qmd`, PLAN §7 Ch 6 / rendered **Chapter 7**) complete (Opus 4.8). Built all 5 §7 items: `fig-pop-projection`, `fig-hh-projection`, `fig-production-need`, `fig-senior-projection`, `tbl-needs-allocation`. **Phase 0:** added `wc_cap(release = 2024)` to `_common.R` (UVA Weldon Cooper Center source line); `CLAUDE.md` + `README.md` helper lists updated. **No API pull, no `r/` re-run** — `read_rds()` only from `wcoop.rds` (every figure) with `gaps`/`chas`/`bps` loaded for cross-reference (their gap direction/permit pace are already baked into `wcoop$production` and `wcoop$needs_allocation`). **Household headline = avg-household-size method (+6,795 to 2050 → 33,515 HH; Jonathan 2026-07-10);** headship (+6,662 → 33,382) shown as a faint dashed sensitivity line — the two methods differ by only ~133 HH, so the chart shows they nearly coincide. **Key numbers:** Fauquier pop 74,577 (2024) → **93,171 (2050)**, +18,594 / +24.9% (2030→2050 +20.1%, outpacing VA's +14.2%); Warrenton 10,693 (2030) → **12,841 (2050)**, +20.1%; seniors **65+ 17,157→18,246→18,208** and **75+ 7,371→9,939→9,844** across 2030/2040/2050 (surge-then-plateau; 75+ share of 65+ rises 43%→54%); production need **~269 units/yr through 2050** (204/yr near-term through 2030) vs **266/yr** 2020–25 permit pace; needs allocation sums to **+6,795** (owner 5,267 / 77% — incl. 3,566 in the >100% AMI band — vs renter 1,528 / 23%). **Production-need GP divergence framed, not hidden (per brief):** takeaway leads with the *actual* finding — "recent building has roughly kept pace with projected household growth" (269 vs 266) — and a dedicated bullet notes this **inverts the GP study's "need exceeds pace" (GP 307/yr, +7,737 HH)**, attributing the gap to the household-size assumption (this study's actual 2020–2024 ACS avg **2.78** vs GP's back-implied ~2.70; larger size → fewer new HH for the same population → lower annual need), with full reconciliation deferred to the appendix (S11b). **Judgment calls / spec resolutions:** (1) `fig-pop-projection` built as a 2-panel `facet_wrap(~geo, scales="free_y")` (Fauquier + Warrenton) using the 2030/2040/2050 decade points to match the town frame's grain, satisfying the brief's "small secondary panel"; (2) `fig-hh-projection` prepends a 2024 baseline row (26,720 from `assumptions$base_hh`) so the growth story anchors to the current count; (3) `fig-production-need` shows avg-size units/yr columns across all three horizons + a `geom_hline` at the 266/yr permit pace (headship not co-plotted — cited in text only — to keep the column chart clean, since 264 vs 269 would overplot); (4) `fig-senior-projection` is WC-only (2030/2040/2050) with the current-state 65+ profile cross-referenced to @sec-populations rather than re-read; (5) **`tbl-needs-allocation` renter top bands collapsed** — the frame's renter `ami100`+`ami120` rows carry the *same* `-2,256` existing gap (the rental-gap frame's coarser ">80% AMI" band), so they're merged into one ">80% AMI" row (forward_need summed 161+541=702; gap taken **once** via `first()`, never summed) → a 9-row table (5 owner + 4 renter) matching each tenure's native gap scale; `pack_rows("Homeowner"/"Renter")`, forward-need total documented in the footnote. **Callouts:** Warrenton town spotlight (grows +20.1% to 12,841, principal service area; **Bealeton absent from WC** — CDP, not an incorporated large town, folded into county totals); §6 interview-validation callout confirms-and-quantifies-forward the seniors/young-returners claim (65+ +1,089 through 2040 then plateau; 75+ share 43%→54%; ~269 units/yr just to keep pace with formation; ties to @fig-ownership-gap entry barrier). **Gotchas honored (from S9/S10 precedent):** no `@fig`/`@tbl`/`@sec` cross-refs inside `labs(caption=)` or kbl `footnote()` (gridtext `<a>`-tag error + kbl footnotes bypass Quarto crossref resolution) — all cross-refs live in markdown bullets, where they resolve; long subtitles trimmed to fit the freeze-PNG width (a trailing plain word after a bold ggtext span collapses its leading space, so subtitles end on the span); `case_when()`/tidyverse idioms throughout. **R runtime note:** documented R-4.5.1 is absent on this machine — installed are 4.4.2 / 4.5.0 / 4.6.1; rendered under **R 4.6.1** (renv library loads clean: dplyr 1.2.1, ggplot2 4.0.3). **Render:** `quarto render projections.qmd` clean; full-book `quarto render` clean (11 chapters); `projections` sits in report order between `populations` and `conclusions` as **Chapter 7**; `@sec-projections` + internal `@fig-*`/`@tbl-*` resolve, outbound `@sec-gaps`→"Chapter 5" / `@sec-populations`→"Chapter 6" / `@sec-market-ownership`→"Chapter 3" / `@fig-rental-gap`→"Figure 5.4" / `@fig-ownership-gap`→"Figure 5.5" all resolve; **no `?@` anywhere in `docs/`**; `_freeze/projections` + `docs/` rebuilt. **Deferrals:** none — all 5 §7 Ch 6 items built; `conclusions.qmd` + `data-notes.qmd` = S11b. **Carry-forward to S11b (GP-reconciliation / method-divergence notes for the data-notes appendix):** (a) **household projection model** — avg-HH-size (÷2.78) vs constant-headship (0.3583) methods, why 2.78 (actual 2020–2024 ACS B25010) departs from GP's implied ~2.70, and why both land ~264–269/yr below GP's 307/yr; (b) **production model** — WC HH growth + 3% vacancy allowance annualized vs 2020–25 BPS avg (266/yr), and the resulting "keeps pace" vs GP "need exceeds pace" inversion; (c) **needs-allocation join** — tenure-aware (Homeowner←ownership_gap, Renter←rental_gap) and the renter ami100+ami120 → ">80% AMI" band-scale collapse; (d) the forward allocation holds today's income distribution constant, so it maps *where demand arises*, not affordability — and the existing gap dwarfs forward need in the lowest bands. (Plus the still-open S9/S10 appendix items: ACS-vs-CHAS burden denominator, FMR-overstates-local-rents, family-poverty rate source, PIT region-vs-Fauquier scope, Bealeton/manufactured-housing undercount, CPI-less-shelter deflator, MLS published-vs-export count reconciliation.)
- **2026-07-14** — Session 11 **part a-ext** (projections range + scope-ladder extension) complete (Opus 4.8; follow-on to part a, Jonathan-approved analytical extension). **Plan amendment (logged):** part a's §7 Ch 6 specced a single household-projection figure; §10 bars figures beyond §7 without sign-off. Jonathan approved presenting future need as a **range + scope ladder** rather than one "units needed" number — §7 Ch 6 list extended to items 6–7 (`fig-need-range`, `fig-scope-ladder`). **No API pull, no `r/` re-run, no new caption helper** — `read_rds()` only; sourced `wcoop.rds` (every figure) + `gaps.rds` (scope-ladder backlog, direct) + `acs_stock.rds` (total stock/year built, replacement rung) + `decennial.rds`/`acs_demographics.rds` (historical household-size trend). **`fig-need-range` (Fig A — bracket the one assumption that matters):** hold WC 2050 pop fixed (93,171), vary only avg household size; HH = pop ÷ size, units = ΔHH × 1.03 vacancy ÷ 26 yr. Three anchors — **2.78 (holds) → +6,795 HH → 269/yr; 2.70 (≈GP) → +7,788 → 309/yr; 2.60 → +9,115 → 361/yr** — a **~270–360 units/yr band**; 269 headline framed as the *conservative floor* (sits on the 266/yr permit pace), GP's **307/yr lands inside** (annotated on the mid bar). **Key verification (drove a framing decision, Jonathan-approved):** the brief asked to justify the 2.60 endpoint from Fauquier's *own* history — but Fauquier's household size is **flat-to-rising, not declining**: **2.76 (2010) → 2.76 (2020)** (decennial pop ÷ occupied HH, H004001/H4_001N) **→ 2.78 (2024 ACS B25010)**. So 2.60 is *not* a local-trend extrapolation; the high end is reframed explicitly as a **convergence sensitivity** — "if Fauquier begins following the national/state decline it has so far resisted" (VA avg = 2.52 cited as the realistic floor). Decision: **keep 2.60, reframe as convergence** [Jonathan 2026-07-14]. **`fig-scope-ladder` (Fig B — components-of-need building bar, cumulative, reversed stack so growth is the base):** four rungs — (1) **new households 6,795** → (2) **+ 3% vacancy → 6,999** (the published headline) → (3) **+ existing backlog → 10,597** → (4) **+ replace aging stock → 12,457** (~261 / 269 / 408 / 479 units/yr). **Rung-3 backlog definition (Jonathan-approved) = data-grounded + assumption-flagged split:** rental **1,059** (sum of sub-80%-AMI `rental_gap` surplus_deficit: 625+164+270 — a true affordable-*and-available* CHAS deficit, data), **plus ownership 2,539** = **one-third of the 7,616 sub-80%-AMI ownership shortfall** (a conservative "buildable share"; the raw `ownership_gap` is a for-sale *snapshot* — households − active listings — not a build target, so the −15,469 top band and even the −7,616 face value are **not** used) [Jonathan 2026-07-14]. **Rung-4 replacement = assumption-driven, flagged:** **0.25%/yr × 28,621-unit stock × 26 yr = 1,860 units** — no demolition data exists (BPS = permits/additions only), so it is stated, not measured; justified by aging stock (median year built **1987**, **56% pre-1990** — corrects an earlier session note's "56% pre-1980", which is actually 35%). Legend labels the two assumed segments "(assumed)"; the two rightmost rungs are called out in bullets as *scope, not precision*. **Data-grounded vs assumed:** grounded — Fig A whole range (pop × size arithmetic), rungs 1–2, rung-3 rental 1,059; assumed — rung-3 ownership 2,539 (the 1/3 haircut), rung-4 replacement 1,860 (rate + no demolition data). **Gotchas honored:** no cross-refs inside `labs(caption=)` (gridtext `<a>`-tag) — `@fig-rental-gap` kept in a bullet; titles shortened to fit the freeze-PNG one-line width (calibrated ~68-char titles fit; the initial 84/94-char titles clipped) and subtitles end on the ggtext span; Fig A bar-1 label collision fixed by dropping the coral permit-pace annotation below its line (`vjust = 1.6`); Fig B stack reversed (`position_stack(reverse = TRUE)`) so "New households" is the base; `case_when`/`.by=`/native-pipe idioms; rendered under **R 4.6.1** (renv clean). **Render:** `quarto render projections.qmd` clean; both PNGs visually verified (numbers, labels, subtitles all fit); full-book `quarto render` clean (11 chapters); **no `?@` in `docs/`**; `_freeze/projections` + `docs/` rebuilt. Figures inserted after `fig-production-need` (new sections "How much does the answer swing?" and "From new arrivals to full need"). **Deferrals:** the S11b data-notes appendix should document (a) Fig A's range method + the flat-Fauquier-size finding that makes 269 the floor and reframes 2.60 as convergence, and (b) Fig B's backlog definition (data vs assumed split) + the 0.25%/yr replacement assumption — added to the S11a carry-forward list.
